...
float * normalize_for_trt(const cv::cuda::GpuMat &img)
{
	...
	cv::resize(img, resizedMat, size, 0, 0, INTER_LINEAR);
...
float * normalize_for_trt(const Mat &img)
{
	cv::Size size(INPUT_W, INPUT_H);
	//cuda::GpuMat resizedMat;
    
    Mat resizedMat;

	cv::resize(img, resizedMat, size, 0, 0, INTER_LINEAR);
	Mat flatData = resizedMat.reshape(1, 1);

	unsigned volChl = INPUT_H * INPUT_W;

    std::cout << "Size of float array: " << INPUT_C * volChl << std::endl;
	float * data;
	cudaMalloc((void**)&data, INPUT_C * volChl * sizeof(float));
	data = (float *)flatData.data;
...
int main() {
	...
	// read in image
	Mat input = cv::imread("./image/elephant.jpg");
	if (input.empty()){
	    std::cout << "Faile imread()" << std::endl;
	}
	std::cout << "Image dimensions: " << input.size() << std::endl;
	std::cout << "Image channels : " << input.channels() << std::endl;
    ...
    std::cout << "Allocated input and ouput memory" << std::endl;
    ...
    std::cout << "Successfully performed inference" << std::endl;
	float max_value = -100;
	int max_index = -1;
	...
	for (int j = 0; j < OUTPUT_LEN; j++){
	    ...
	    if (j==387 || j==388) {
	        std::cout << "Elephant probability: " << output[j] << std::endl; }
		if (output[j] > max_value){
			max_value = output[j];
			max_index = j;
		}
	}
	// PRINTING THE MAX VALUE INDEX e.g. index 3 = parrot	
